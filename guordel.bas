...
#autostart
  ...
  10 LET I=NOT PI:LET P=VAL"7":LET O=VAL"4":LET K=VAL"2":LET Y=VAL"6":LET Q=NOT PI:LET G=NOT PI
  20 LET D=VAL"43897"-VAL"1":LET A=D+VAL"6":LET B=A+VAL"1":LET C=VAL"35840":LET E=VAL"39168":LET F=VAL"43840":GO TO 900
 ...
 550 LET T=T+VAL"1":BORDER P:FOR J=VAL"1" TO VAL"5":LET S=K:IF G$(J TO J)=W$(J TO J) THEN LET S=O:LET H$(J TO J)=W$(J TO J):GO TO 580
 560 FOR L=VAL"1" TO VAL"5":IF G$(J TO J)=W$(L TO L) THEN LET S=Y
 570 NEXT L
 580 POKE VAL"22636"+T*VAL"32"+J,I*VAL"8"+S:LET H=0:FOR L=1 TO LEN(U$):IF U$(L TO L)=G$(J TO J) THEN LET H=L
 590 NEXT L:IF V(H)<>O THEN LET V(H)=S
 600 POKE U(H),VAL"8"*I+V(H):NEXT J:INK I:PAPER P:IF G$=W$ THEN BORDER P:INK O:PRINT G$:INK O:PRINT AT VAL"11",VAL"10";"SUCCESS! :-)":INK I:GO TO 630
 610 IF T<VAL"6" THEN GO TO 310
 620 LET T=NOT PI:INK K:PRINT AT VAL"11",VAL"11";"GAME  OVER":INK I:PRINT AT VAL"12",VAL"9";"IT WAS: ";W$
 ...
 899 REM Load Stats
 900 FOR J=NOT PI TO VAL"55":POKE F+J,NOT PI:NEXT J:IF G<>NOT PI THEN GO TO 970
 910 INK P:PRINT AT VAL"12",VAL"0";"LOAD STATISTICS FROM TAPE? (Y/N)"
 ...
 980 GO TO 30
 990 INK P:PAPER VAL"1":BORDER VAL"1":CLS:PRINT AT VAL"10",VAL"2";"Dedicado a Sara y Montse :-*":PAUSE 0:INK I:PAPER P:BORDER P:CLS:GO TO 60
...
1080 PAPER I:INK O:PRINT "W";:PAPER P:INK I:PRINT"EARY"
1090 PRINT "THE LETTER W IS IN THE WORD AND"
1100 PRINT "IN THE CORRECT SPOT.":PRINT
1110 PRINT "P";:PAPER I:INK Y:PRINT "I";:PAPER P:INK I:PRINT "LLS"
1120 PRINT "THE LETTER I IS IN THE WORD BUT"
1130 PRINT "IN THE WRONG SPOT.":PRINT
1140 PRINT "VAG";:PAPER I:INK K:PRINT "U";:PAPER P:INK I:PRINT "E"
